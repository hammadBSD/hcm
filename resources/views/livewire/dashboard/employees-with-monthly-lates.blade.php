            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4">
                <div>
                    <flux:heading size="lg" class="text-zinc-900 dark:text-zinc-100">
                        {{ __('Attendance: monthly lates') }}
                    </flux:heading>
                    <flux:text class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        {{ __('Active employees with at least one late day from month start through yesterday (today excluded).') }}
                    </flux:text>
                </div>
                                    ...
                                    @foreach($rows as $row)
                                        <tr class="hover:bg-zinc-100 dark:hover:bg-zinc-600">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                                <flux:badge color="amber" size="sm">{{ $row['late_count'] }}</flux:badge>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">
                        {{ __('No employees with late days in this range.') }}
                    </p>
                </div>
            @endif
        </div>
