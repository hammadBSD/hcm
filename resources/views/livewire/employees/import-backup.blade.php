                        ...
                        <div class="flex items-center justify-between">
                            ...
                            @if($importing)
                                <div class="text-sm text-blue-600 dark:text-blue-400">
                                    ...
                                    <div class="space-y-2">
                                        <!-- Progress Bar -->
                                        <div class="w-64 bg-gray-200 rounded-full h-3 dark:bg-gray-700">
                                            <div id="progress-bar" class="bg-blue-600 h-3 rounded-full transition-all duration-500" 
                                                 style="width: 0%"></div>
                                        </div>
                                        
                                        <!-- Progress Stats -->
                                        <div id="progress-stats" class="text-xs space-y-1">
                                            <div>Processing row <span id="current-row">0</span> of <span id="total-rows">0</span></div>
                                            <div class="flex gap-4">
                                                <span class="text-green-600">✓ <span id="success-count">0</span> successful</span>
                                                <span class="text-red-600">✗ <span id="error-count">0</span> errors</span>
                                            </div>
                                            <div>Batch <span id="current-batch">0</span> of <span id="total-batches">0</span> | <span id="percentage">0</span>% complete</div>
                                        </div>
                                    </div>
                                </div>
                            @endif
...
document.addEventListener('livewire:init', () => {
    Livewire.on('import-progress', (data) => {
        // Update progress bar
        ...
        if (currentRow) currentRow.textContent = data.processed;
        if (totalRows) totalRows.textContent = data.total;
        if (successCount) successCount.textContent = data.success;
        if (errorCount) errorCount.textContent = data.errors;
        if (currentBatch) currentBatch.textContent = data.currentBatch;
        if (totalBatches) totalBatches.textContent = data.totalBatches;
