<section class="w-full">
    ...
    <x-system-management.layout :heading="__('Cost Center')" :subheading="__('Manage cost centers')">
        <div class="space-y-6">
            <div class="flex justify-between items-center gap-4">
                <div class="flex items-center gap-3">
                    <flux:input type="search" wire:model.live="search" placeholder="{{ __('Search cost centers...') }}" class="w-80" />
                    <flux:button variant="outline" icon="funnel" />
                </div>
                <div class="flex items-center gap-3">
                    <flux:button variant="outline" icon="arrow-down-tray">Export</flux:button>
                    <flux:button variant="primary" icon="plus" wire:click="create">{{ __('Add Cost Center') }}</flux:button>
                </div>
            </div>
                                    ...
                                    <td class="px-6 py-6 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            <div class="h-8 w-8 bg-blue-100 dark:bg-blue-900 rounded-lg flex items-center justify-center">
                                                <flux:icon name="building-library" class="h-4 w-4 text-blue-600 dark:text-blue-400" />
                                            </div>
                                            <div>
                                                <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ $costCenter->name }}</div>
                                                @if($costCenter->code)
                                                    <div class="text-xs text-zinc-500 dark:text-zinc-400">{{ $costCenter->code }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-6">
                                        <div class="text-sm text-zinc-900 dark:text-zinc-100">{{ $costCenter->description ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-6 whitespace-nowrap">
                                        <flux:badge color="{{ $costCenter->status === 'active' ? 'green' : 'red' }}" size="sm">{{ ucfirst($costCenter->status) }}</flux:badge>
                                    </td>
                                    <td class="px-6 py-6 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-1">
                                            <flux:dropdown>
                                                ...
                                                <flux:menu>
                                                    <flux:menu.item icon="pencil" wire:click="edit({{ $costCenter->id }})">{{ __('Edit') }}</flux:menu.item>
                                                    <flux:menu.item icon="trash" wire:click="delete({{ $costCenter->id }})" wire:confirm="{{ __('Are you sure you want to delete this cost center?') }}" class="text-red-600">{{ __('Delete') }}</flux:menu.item>
                                                </flux:menu>
                                            </flux:dropdown>
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        ...
                                        <flux:heading size="sm" class="mt-2 text-zinc-500 dark:text-zinc-400">{{ __('No cost centers found') }}</flux:heading>
                                        <flux:text class="mt-1 text-zinc-500 dark:text-zinc-400">{{ __('Create a new cost center to get started.') }}</flux:text>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            @if($costCenters->hasPages())
                <div class="mt-6">{{ $costCenters->links() }}</div>
            @endif
        </div>
    ...
    <flux:modal variant="flyout" :open="$showAddFlyout" wire:model="showAddFlyout">
        <div class="p-6">
            <div class="mb-6">
                <flux:heading size="lg">{{ $editingId ? __('Edit Cost Center') : __('Add Cost Center') }}</flux:heading>
            </div>
