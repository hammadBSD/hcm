<section class="w-full">
                        ...
                        <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
                            @forelse($designations as $designation)
                                <tr class="hover:bg-zinc-100 dark:hover:bg-zinc-600 transition-colors duration-150">
                                    <td class="px-6 py-6 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            <div class="h-8 w-8 bg-blue-100 dark:bg-blue-900 rounded-lg flex items-center justify-center">
                                                <flux:icon name="briefcase" class="h-4 w-4 text-blue-600 dark:text-blue-400" />
                                            </div>
                                            <div>
                                    ...
                                    <td class="px-6 py-6 whitespace-nowrap">
                                        <flux:badge color="{{ $designation->status === 'active' ? 'green' : 'red' }}" size="sm">
                                    ...
                                    <td class="px-6 py-6 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-1">
                                            <flux:dropdown>
                                                ...
                                                <flux:menu>
                                                    <flux:menu.item icon="pencil" wire:click="editDesignation({{ $designation->id }})">
                                                        {{ __('Edit Designation') }}
                                                    </flux:menu.item>
                                                    <flux:menu.item icon="trash" wire:click="deleteDesignation({{ $designation->id }})" wire:confirm="Are you sure you want to delete this designation?" class="text-red-600">
    ...
    <flux:modal variant="flyout" :open="$showAddDesignationFlyout" wire:model="showAddDesignationFlyout">
        <div class="p-6">
            <!-- Header -->
            <div class="mb-6">
                <flux:heading size="lg">{{ $editingId ? 'Edit Designation' : 'Add Designation' }}</flux:heading>
            </div>
            ...
            <form wire:submit="submitDesignation" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <flux:field>
                        <flux:label>Designation Code</flux:label>
                        <flux:input 
                            wire:model="designationCode" 
                            placeholder="Enter designation code (optional)"
                        />
                        <flux:error name="designationCode" />
                    </flux:field>
                </div>
