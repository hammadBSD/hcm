...
class Report extends Component
{
    ...
    /**
     * Filled for the duration of processAttendanceData() to avoid per-day DB queries.
     *
    ...
    private function loadHolidaysForMonth($startOfMonth, $endOfMonth)
    {
        ...
        foreach ($holidays as $holiday) {
            if (! $eligibilityService->appliesToEmployee($holiday, $this->employee)) {
                continue;
            }
    ...
    /**
     * The Employee model has both a legacy `department` string column and a `department()` relation.
     * Never use $employee->department for the related Department model — use this helper instead.
     */
    private function employeeRelationDepartment(?Employee $employee = null): ?Department
    {
        $employee = $employee ?? $this->employee;
        if (! $employee instanceof Employee || ! $employee->department_id) {
            return null;
        }

        if ($employee->relationLoaded('department')) {
            $rel = $employee->getRelation('department');

            return $rel instanceof Department ? $rel : null;
        }

        return Department::query()->with('shift')->find($employee->department_id);
    }
    ...
    private function warmAttendanceDayProcessCaches(Carbon $startOfMonth, Carbon $endDate): void
    {
        ...
        if ($this->employee->department_id) {
            if (! $this->employee->relationLoaded('department')) {
                ...
                $dept = $this->employee->getRelation('department');
                if ($dept instanceof Department && ! $dept->relationLoaded('shift')) {
                    $dept->load('shift');
                }
            }
        }
        ...
        while ($cursor->lte($endDate)) {
            ...
            if (! $dayShift && $this->employee->department_id) {
                $department = $this->employeeRelationDepartment();
    ...
    private function processAttendanceData($records)
    {
        ...
        $gracePeriodHours = 5;
        $groupedRecords = AttendancePunchDayGroupingService::groupPunchesByWorkday($this->employee, $records, $gracePeriodHours);
    ...
    public function saveMissingEntry()
    {
        ...
        try {
            if (in_array($this->missingEntryType, ['IN', 'OUT'])) {
                ...
                } elseif ($this->missingEntryType === 'edit_checkin_checkout_exclude_breaks') {
    ...
    /**
     * Dashboard widget: count of workdays with a late present status in the selected month
     * (through yesterday for the current month, else full month), after leave enrichment — matches attendance grid.
     *
     * @return array{count: int}|null
     */
    public function evaluateEmployeeLateDaysForDashboard(Employee $employee, string $ym): ?array
    {
        if ($employee->status !== 'active' || empty($employee->punch_code)) {
            return null;
        }
        ...
        foreach ($data as $row) {
            ...
            if (str_contains($status, 'present_late') && ! $this->isDateExemptedFromLates($d)) {
                $lateDays++;
            }
        }
