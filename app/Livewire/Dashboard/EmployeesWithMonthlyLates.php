...
#[Lazy]
class EmployeesWithMonthlyLates extends Component
{
    /** @var string Month (Y-m); empty means current month */
    public string $selectedMonth = '';

    /** @var list<array{id: int, name: string, employee_code: ?string, department: string, group: string, late_count: int}> */
    public array $rows = [];
    ...
    public function loadRows(): void
    {
        ...
        if (! Auth::user()?->can('dashboard.view.attendance_month_lates')) {
            return;
        }
        ...
        foreach ($employees as $employee) {
            $result = $scanner->evaluateEmployeeLateDaysForDashboard($employee, $ym);
            if ($result === null) {
                continue;
            }
    ...
    {
        return view('livewire.dashboard.employees-with-monthly-lates', [
