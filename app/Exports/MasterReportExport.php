    ...
    public function array(): array
    {
        ...
        foreach ($groups as $group) {
            ...
            $rows[] = [
                ...
                'Basic Salary', 'Allowances', 'Gross Salary', 'Hourly Rate', 'Daily Rate', 'Hourly Deduction Amount', 'Lates Adjustment', 'Deduction Late Days', 'Deduction Absent Days', 'Salary Deduction', 'Net Salary', 'Bonus',
            ...
            foreach ($group['employees'] as $r) {
                ...
                $rows[] = [
                    ...
                    ($r['late_adjustment_days'] ?? 0) > 0 ? $r['late_adjustment_days'] : '—',
                    $r['deduction_late_days'] ?? 0,
                    number_format($r['deduction_absent_days'] ?? 0, 2),
                    number_format($r['other_deductions'] ?? 0, 2),
                    number_format($r['net_salary_after_attendance'] ?? 0, 2),
                    number_format($r['bonus'] ?? 0, 2),
                    number_format($r['tax'] ?? 0, 2),
                    number_format($r['tax_adjustment'] ?? 0, 2),
        ...
        $rows[] = array_merge(
            ['GRAND TOTAL'],
            array_fill(0, 25, ''),
            ['', number_format($grandGross, 2), number_format($grandBasic, 2), '', '0', '0.00', '', '0.00'],
            array_fill(0, 5, '0.00'),
